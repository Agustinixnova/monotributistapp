-- =====================================================
-- FIX: Resolver overloading de función caja_crear_cliente_con_saldo
-- El problema es que hay dos versiones con firmas similares (TEXT vs VARCHAR)
-- que PostgreSQL no puede distinguir al llamar con parámetros named
-- =====================================================

-- Drop TODAS las versiones posibles de la función
DROP FUNCTION IF EXISTS public.caja_crear_cliente_con_saldo(UUID, VARCHAR, VARCHAR, VARCHAR, DECIMAL, TEXT, DECIMAL, VARCHAR, UUID);
...
DROP FUNCTION IF EXISTS public.caja_crear_cliente_con_saldo;

-- Crear una única versión definitiva
CREATE OR REPLACE FUNCTION public.caja_crear_cliente_con_saldo(
  ...
  p_tipo_saldo VARCHAR(10) DEFAULT NULL,
...
BEGIN
  ...
  IF p_saldo_inicial IS NOT NULL AND p_saldo_inicial > 0 AND p_tipo_saldo IS NOT NULL THEN
    ...
    IF p_tipo_saldo = 'deuda' OR p_tipo_saldo = 'debe' THEN
