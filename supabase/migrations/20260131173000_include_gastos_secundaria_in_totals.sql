-- Incluir gastos de caja secundaria en el total del día
-- Los gastos son egresos reales del negocio, aunque se paguen desde caja secundaria
...
BEGIN
    ...
    WITH movimientos_dia AS (
        ...
        -- Gastos de caja secundaria (son egresos reales)
        SELECT
            'salida' as tipo,
    ...
    efectivo AS (
        -- Efectivo de caja principal (excluyendo transferencias internas)
        SELECT
    ...
    gastos_secundaria_efectivo AS (
        -- Gastos de caja secundaria (siempre son efectivo)
        SELECT
            COALESCE(SUM(cs.monto), 0) as gastos_efe
        FROM public.caja_secundaria_movimientos cs
        WHERE cs.user_id = p_user_id
        AND cs.fecha = p_fecha
        AND cs.tipo = 'salida'
    ...
    SELECT
        ...
        t.cant,
        e.efe_entradas,
        e.efe_salidas + g.gastos_efe as efectivo_salidas_total, -- Incluir gastos de secundaria
        e.efe_entradas - (e.efe_salidas + g.gastos_efe) as efe_saldo,
        d.dig_entradas,
        d.dig_salidas
    FROM totales t, efectivo e, digital d, gastos_secundaria_efectivo g;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;

COMMENT ON FUNCTION public.caja_resumen_dia_v2 IS
'Calcula el resumen del día excluyendo transferencias internas pero INCLUYENDO gastos de caja secundaria';
...
BEGIN
    ...
    SELECT
        ...
        (e.efe_salidas + g.gastos_efe)::DECIMAL(12,2), -- Incluir gastos de secundaria
        (e.efe_entradas - (e.efe_salidas + g.gastos_efe))::DECIMAL(12,2),
...
'Resumen del día para el dueño de la caja, excluyendo transferencias internas pero INCLUYENDO gastos de caja secundaria';
