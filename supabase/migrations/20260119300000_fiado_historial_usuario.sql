-- Actualizar función caja_cliente_historial para incluir usuario que registró el fiado

-- Primero eliminar la función existente
DROP FUNCTION IF EXISTS public.caja_cliente_historial(UUID);

-- Recrear con nuevos campos
...
BEGIN
  ...
  SELECT
    ...
    COALESCE(p.full_name, u.email, 'Usuario')::VARCHAR(255) AS created_by_nombre
  ...
  SELECT
    ...
    COALESCE(p2.full_name, u2.email, 'Usuario')::VARCHAR(255) AS created_by_nombre
