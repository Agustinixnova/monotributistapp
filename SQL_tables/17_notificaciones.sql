...
-- SELECT: Admin y desarrollo pueden ver todas (debugging)
CREATE POLICY "notificaciones_select_admin" ON public.notificaciones
    FOR SELECT USING (
        public.get_user_role() IN ('admin', 'desarrollo')
    );

-- INSERT: SOLO via función SECURITY DEFINER (no directo desde cliente)
-- NO creamos policy de INSERT aquí, se hace via función
-- Esto evita que un cliente pueda crear notificaciones falsas
...
-- Solo puede ser llamada desde el servidor (service role) o funciones internas
-- =============================================
...
REVOKE EXECUTE ON FUNCTION public.crear_notificacion FROM PUBLIC;
REVOKE EXECUTE ON FUNCTION public.crear_notificacion FROM anon;
REVOKE EXECUTE ON FUNCTION public.crear_notificacion FROM authenticated;
...
-- =============================================
-- Función para marcar notificación como leída (callable por usuario)
-- =============================================
