 ...
 * Inserta en client_audit_log (tabla legacy de auditoria)
 *
...
export async function registrarCambio({
  ...
  try {
    // Insertar en la tabla de auditoria legacy (client_audit_log)
    const { data, error } = await supabase
      .from('client_audit_log')
      .insert({
        client_id: clientFiscalDataId,
        tabla: 'client_fiscal_data',
        campo: campo,
        valor_anterior: valorAnterior || null,
        valor_nuevo: valorNuevo || null,
        modified_by: realizadoPor,
        motivo: metadata.motivo || null,
        origen: metadata.origen || 'manual'
      })
      .select()
      .single()
...
export async function obtenerHistorialCambios(clientFiscalDataId, options = {}) {
  ...
  try {
    let query = supabase
      ...
      .select(`
        *,
        modified_by_profile:profiles!modified_by(nombre, apellido, email)
      `)
    ...
    return (data || []).map(item => ({
      id: item.id,
      tipo_cambio: tipoCambio || TIPO_CAMBIO.OTROS,
      campo: item.campo,
